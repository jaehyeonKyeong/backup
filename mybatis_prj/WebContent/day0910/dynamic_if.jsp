...
<form action="use_select_mybatis.jsp" method="get">
	<input type="hidden" name="page" value="day0910/dynamic_if">
	<label>조회할 부서 선택</label>
...
<%
	...
	List<DynamicIf> empList = se3.dynamicIf(Integer.parseInt(deptno));
		...
		<table>
			...
			<c:set var="i" value="${i+1 }"/>
			 <tr>
			 	...
			 	<td>${emp.sal }</td>
			 	<td>${emp.hiredate }</td>
			 	<td>${emp.deptno }/${param.deptno >= 10 or param.deptno < 40?param.deptno:""}</td>
			</tr>
			</c:forEach>
		</table>
	</div>
