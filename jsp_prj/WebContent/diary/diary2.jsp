...
#diaryFrm{ position: absolute;top: 90px;left: 80px;
				...
				box-shadow: 10px 10px 10px #333;display: none }					
/* diary 관련 디자인 끝 */			
</style>
<!--  summer note 시작 -->
...
<script type="text/javascript">
	...
	$(function(){
			...
			$("td > div > a").click(function() {
				...
				var temp=$("#nowDate").text();
				$("#param_year").val( temp.split(".")[0] );
				$("#param_month").val( temp.split(".")[1] );
				
				$("#param_day").val( $(this).find("span").text() );
			});
...
<body>
          ...
          <ul class="mega-menu">
            <li>
              ...
              <div style="width:400px;max-width:100%;">
                ...
                <p>일정관리 시스템</p>
                <p>고객의 일정을 보다 정확하고 신속하게 관리함을 목적으로</p>
                <p>제작된 프로그램입니다.</p>
                </div>
              </div>
            </li>
	...
	//현재월에 없는 날짜가 입력되면 다음달 1일로 넘어가는 것을 막는다. 
				...
				<div>
				 <a href="#" class="writeDiary"><span class="<%= dayColor%>"><%= tempDay %></span>
				 </a>
				</div>
				<%	if(evtList != null){ %>
				<div>
					<% for(MonthVO mv: evtList){	%>
						<img src="images/evt.png" title="<%= mv.getSubject() %>"/>
					<% 	}//end for %>
				</div>
				<%	}//end if	%>				
				</td>
...
<table>
	<tr>
		<td colspan="2" class="wTitle">이벤트작성</td>
	</tr>
	<tr>
		<td class="wLabel">작성자 </td>
		<td class="wInput"><input type="text" name="writer" class="inputBox"/></td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="2"  class="wTitle">
			<input type="button" value="이벤트작성" class="btn"/>
