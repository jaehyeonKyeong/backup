...
public class LoginEvt implements ActionListener {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == lg.getBtnLogin()) {
			exceptionHandling(l_dao);
		}
	}//actionPerformed
	
	public void exceptionHandling(LoginDAO l_dao) {
		if (lg.getRbUser().isSelected()) {
			...
			try {
				...
				if (uiv != null) {
					...
					JOptionPane.showMessageDialog(lg, "로그인 실패 아이디와 패스워드를 확인해주세요!!", "로그인 실패! :(",
			...
			if (!(lg.getRbUser().isSelected() || lg.getRbManager().isSelected())) {
				JOptionPane.showMessageDialog(lg, "사용자 또는 사업자를 선택해주세요!!", "로그인 실패! :(",
						JOptionPane.WARNING_MESSAGE);
			}
		}
	}
